...
char** av;
{
	...
	bool	gotone = false;
	/*char	*p; --POR MMC */
	char* cmd_line_term = 0/*, term_1stch*/; /* POR MMC */
	int		error2;
		...
		if (i == 1) {
			...
			for (tv = av + 1, i = 0; *tv; tv++)
				if (**tv != Spice_OptChar) {
					...
					while ((i = (int)fread(buf, 1, BSIZE_SP, tp)) > 0)
...
bool terse;
{
	wordlist* coms/*, *command*/; /* POR MMC */
	char /* *plottype,*/* s = NULL; /* POR MMC */
	/*struct dvec *v; --POR MMC */
	...
	/* POR MMC
		...
		if (ciprefix(".print", coms) || ciprefix(".plot", coms) ||
			ciprefix(".four", coms))
		{
			fprintf(stderr,
